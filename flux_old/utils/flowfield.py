...
def setup_args(args: SimpleNamespace):
    for arg in list(args.__dict__):
        ...
        elif hasattr(prop, 'type') and prop.type == cfg.TYPE_FILE_DIALOG:
            dpg.add_file_dialog(width=cfg.sp_width/2, label=arg, parent=cfg.ff_func_settings, default_value=prop.val, user_data=prop, callback= lambda sender, data, property: getattr(property, 'callback')(sender, data, property) if hasattr(property, 'callback') else setattr(property, 'val', data))
        else:
