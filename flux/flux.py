...
def init_frame_buffer(sender, buffer):
    ...
    with dpg.mutex():
        ...
        dpg.configure_item('parameters', width=w_width-cfg.ff_width)
        # Setup Initial Frame
        ...
        dpg.add_image('prev-frame-texture',tag='prev-frame', width=cfg.ff_width, parent='flowfield', pos=(0,0), uv_min=(0,0), uv_max=(cfg.ff_width/w_width, 1))
...
def start_flux():
    # Start Flux
    dpg.create_context()
    dpg.create_viewport(title='Flux', width=cfg.ff_width+cfg.sp_width, height=cfg.ff_height, resizable=False)
    dpg.setup_dearpygui()
    setup_flux()
    dpg.show_viewport()
