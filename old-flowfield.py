...
def setup_flowfield(rows, cols, scale):
    ...
    width = cols * scale
    height = rows * scale +20

    with dpg.texture_registry():
        dpg.add_dynamic_texture(width=dpg.get_viewport_client_width(), height=dpg.get_viewport_client_height(), default_value=[0,0,0,0]*width*height, tag="texture_tag")
    with dpg.window(label="FlowField", tag='grid_display', width=width, height=height, no_title_bar=True):
        dpg.set_primary_window('grid_display', True)
        ...
        # with dpg.drawlist(width=cols*scale, height= rows*scale, tag='grid') as grid:
            # for y in range(rows):
            #     for x in range(cols):
                    # idx = x + y * cols
            ...
            #     particles[i] = Particle(parent = grid, pos = [random.random() * cols * scale, random.random() * rows * scale])
            #     particles[i].show()
...
def handle_frame_buffer(sender, buffer):
    print(4*ROWS*COLS*SCALE*SCALE, len(buffer))
    print(len(buffer) == 4 * ROWS * COLS * SCALE * SCALE)
    dpg.set_value('texture_tag', buffer)

dpg.create_viewport(title='Custom Title', width=COLS*SCALE, height=ROWS*SCALE)
dpg.setup_dearpygui()
dpg.show_viewport()
# dpg.show_metrics()
...
while dpg.is_dearpygui_running():
    ...
    dpg.output_frame_buffer(callback=handle_frame_buffer)
